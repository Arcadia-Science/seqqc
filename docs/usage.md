...
```bash
nextflow run Arcadia-Science/seqqc --input samplesheet.csv --outdir <OUTDIR> --genome GRCh37 -profile docker
```
...
First, go to the [Arcadia-Science/seqqc releases page](https://github.com/Arcadia-Science/seqqc/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
